...
def get_item_id_from_table_id_and_index(table_id, item_index):
    ...
    item_id, _, _ = db.session.query(item_menus).filter(
        item_menus.c.menu_id == menu_id,
        item_menus.c.index_in_menu == item_index,
    ).one()
...
def get_item_index_from_table_id_and_id(table_id, item_id):
    ...
    _, _, item_index = db.session.query(item_menus).filter(
        item_menus.c.menu_id == menu_id,
        item_menus.c.item_id == item_id,
    ).one()
