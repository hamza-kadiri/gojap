...
def create_jap_event_service(data):
    ...
    Args :
        data = {userName, description, jap_place_id, userName, date}
    """
    jap_event = JapEvent(userName=data['userName'],
    ...
    join_jap_event_service({'userName': data['userName'],
...
def join_jap_event_service(data):
    ...
    Args :
        data = {userName, jap_event_id}

    Returns :
        {userName, jap_event_id, jap_tables, jap_members}
    """
    japs[data['jap_event_id']].append(data['userName'])
    ...
    user = User.query.filter_by(id=data['userName'])
